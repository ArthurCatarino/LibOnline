...
async function cadastro(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ resposta: `Livro: ${livro.titulo} cadastrado com sucesso` });
  } catch (error) {
    console.error("Erro ao adicionar filme", error);
    let codigoHTTP = 500;
    if (error.message == "Esse livro ja existe" || error.deatails) {
      codigoHTTP = 400;
    }
    return res
      .status(codigoHTTP)
...
async function listagemUnica(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ mensagem: "Erro ao listar livro", erro: erro.message });
  }
}
...
async function editar(req, res) {
  try {
    ...
    let codigoHTTP = 500;
    if (error.message == "Id do livro nao encontrado" || error.details) {
      codigoHTTP = 400;
    }
    console.error("Erro ao atualizar livro", error);
    res
      .status(codigoHTTP)
      .json({ mensagem: "Erro ao atualizar livro", erro: error.message });
  }
}
...
async function deletar(req, res) {
  try {
    ...
    let codigoHTTP = 500;
    if ((error.message = "Id do livro nao encontrado")) {
      codigoHTTP = 400;
    }
    console.error("Erro ao deletar livro", error);
    res
      .status(codigoHTTP)
      .json({ mensagem: "Erro ao deletar livro", erro: error.message });
  }
}
